...
import {database, DataSnapshot} from '../admin';
...
const pastWordsRef = database.ref('wordOfTheDay').child('past');
const currentWOTDRef = database.ref('wordOfTheDay').child('current');
const wordSelection = database.ref('wordOfTheDay').child('selection');

// Function to select a random word every day at 00:00 Dublin time.
export const generateWOTD = functions.pubsub.schedule('00 00 * * *')
